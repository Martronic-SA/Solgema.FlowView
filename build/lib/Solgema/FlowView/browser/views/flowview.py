...
function resizePages(container) {
    ...
    $(panes).children().each( function(index) {
...
function runFlowView(container) {
    ...
    for (i=1;i<%(batch_size)i;i++) {
        var cloned = container.find("#"+itemid+"-pane"+i).clone().addClass('cloned').appendTo("#"+itemid+" #flowpanes .items:first");
    }
    %(effectjs)s
""" % {
...
//        mousewheel : true,
    ...
    def __call__(self):
        self.settings = IFlowViewSettings(self.context)
        return self.index()
